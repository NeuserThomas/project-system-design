...
public class CinemaRestController {
	...
	/**
	 * Only get the cinema names. (For drop down menu)
	 * @return
	 */
	@GetMapping("/getAllByName")
	public @ResponseBody ResponseEntity<List<String>> getCinemaNames() {
		List<String> cinemas = cinemaRepo.findAllCinemaNames();
		if (cinemas!=null) {
			return new ResponseEntity<List<String>>(cinemas, HttpStatus.OK);
		} else {
	...
	public @ResponseBody ResponseEntity<Cinema> getByName(@PathVariable String name) {
		logger.info("Rest call: getOneByName:" + name);
		Optional<Cinema> cinema = cinemaRepo.findOneByCinemaName(name);
		if (cinema.isPresent()) {
			return new ResponseEntity<Cinema>(cinema.get(), HttpStatus.OK);
		} else {
			return new ResponseEntity<Cinema>(HttpStatus.NOT_FOUND);
	...
	public @ResponseBody ResponseEntity<List<Movie>> getPlannedMovies(@PathVariable long cinemaId) {
		...
		if (cinema.isPresent()) {
			...
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Cinema with id " + cinemaId + " not found!");
		}
	}
	...
	public @ResponseBody ResponseEntity<List<Movie>> unPlanMovie(@PathVariable long cinemaId, @RequestBody Movie m) {
		...
		if (cinema.isPresent()) {
			...
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "The movie was never planned!");
		}
	}
